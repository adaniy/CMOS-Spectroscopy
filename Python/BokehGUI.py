...
def run_button_handler(attr, old, new):
    ...
    os.system("python3 CMOSProcess.py --jpg=" + str(settings['save_jpg']) + " --np=" +           str(settings['save_np']) + " --t=" + str(settings['threshold']) + " --ft=" +            str(settings['find_threshold_bool']) + " --multi="+ str(settings['multi']) + " --num=" + str(settings['num_images']) + " --std=" + str(settings['std']))
...
img = img[:,:,0]
x_values = np.arange(img.shape[1] * img.shape[0])
print("Flattening...") # FIXME
y_values = img.flatten()
# output to static HTML file (with CDN resources)
output_file("CMOS_Sensor.html", title="BokehGUI.py example", mode="cdn")

TOOLS = "crosshair,pan,wheel_zoom,box_zoom,reset,box_select,lasso_select"
# create a new plot with the tools above, and explicit ranges
p = figure(tools=TOOLS, match_aspect=True, background_fill_color="#440154")
print("Creating figure...") #FIXME
p.circle(x=x_values, y=y_values, size=0.1, line_color=None, fill_color='navy')
print("done")
'''
